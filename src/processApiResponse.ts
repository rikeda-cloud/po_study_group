...
async function processOkResponse(rawUsers: RawUser[]): Promise<ResponseData> {
  ...
  const filteredRawUsers = rawUsers.filter((rawUser: RawUser) => rawUser.birthday !== null && (rawUser.is_active === true || rawUser.is_active === 1));

  const users: User[] = [];
  for (const rawUser of filteredRawUsers) {
    const user_id: number = Number(rawUser.user_id);
    const friend_ids: number[] = rawUser.friend_ids === null ? [] : rawUser.friend_ids.map((id: string | number) => Number(id));
...
function convertRawBirthdayToBirthday(rawBirthday: string | number | null): Date {
