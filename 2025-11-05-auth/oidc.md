...
- **OpenIDプロバイダ(OP)**: `{認可サーバ}`に相当し、ユーザー認証を行い、IDトークンを発行します。
- **リライングパーティ(RP)**: `{クライアント}`に相当し、OPに対してユーザー認証を要求し、その結果を受け取ります。
...
- **IDトークン**:
    ...
    - OAuth 2.0の認可リクエストの`{scope}`に`{openid}`を含めることで、IDトークンが発行されるようになる。

- **UserInfoエンドポイント**:
    - IDトークンには基本的な情報しか含まれない場合に、追加のユーザー情報を取得するためのエンドポイントです。このエンドポイントへのアクセスにはOAuth 2.0の`{アクセストークン}`が必要です。
...
1.  **認証リクエスト**:
    - RPは、OAuth 2.0の認可リクエストの`{scope}`パラメータに`{openid}`を追加して、ユーザーをOPへリダイレクトさせます。
...
3.  **認可コードの払い出し**:
    - OPはユーザーをRPの`{redirect_uri}`にリダイレクトさせ、`{QueryString}`に認可コードを付与します。

4.  **トークンリクエスト**:
    - RPは受け取った`{認可コード}`を使い、OPの`{トークンエンドポイント}`にリクエストを送信します。このリクエストには、`{クライアントID}`と`{クライアントシークレット}`も含まれます。

5.  **IDトークンとアクセストークンの発行**:
    - OPは`{認可コード}`を検証し、問題がなければRPに`{IDトークン}`と`{アクセストークン}`を発行します。ここがOAuth 2.0との大きな違いです。

6.  **IDトークンの検証とユーザー情報の取得**:
    - RPは受け取った`{IDトークン`}の署名や有効期限などを検証し、改ざんされていないことを確認します。
