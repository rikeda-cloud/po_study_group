...
1. **認可リクエスト**:
    ...
    - **具体的なリダイレクト処理**:
        - クライアントは、ユーザーのブラウザを以下の形式のURLにリダイレクトします。
        ```
        GET {認可サーバの認可エンドポイント}?
            response_type=code&
            client_id={クライアントID}&
        ...
        - 各パラメータの意味:
            ...
            - `client_id`: クライアントアプリケーションを識別するためのIDです。認可サーバに事前に登録されています。
            - `redirect_uri`: 認可コードが返されるクライアントのURIです。認可サーバに事前に登録されている必要があります。
            - `scope`: クライアントがアクセスを要求するリソースの範囲を指定します（例: `profile`, `email`, `openid`など）。
            - `state`: CSRF攻撃を防ぐためのランダムな文字列です。認可リクエスト時にクライアントが生成し、認可レスポンスで返される値と照合します。
